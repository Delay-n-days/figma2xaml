...
作为 WPF 开发者，你是否遇到过这些痛点：

- 😫 **手写 Grid 布局太痛苦**：反复计算 `Grid.Row`、`Grid.Column`，调试对齐和间距
- 😫 **嵌套容器心智负担重**：分析设计稿该用 Grid 还是 StackPanel，层级关系容易出错
- 😫 **Visual Studio 设计器不好用**：拖拽式设计器效率低，生成的 XAML 代码质量差
- 😫 **设计稿变更重新来过**：设计师改了 Figma，开发者要手动重写大量布局代码
- 😫 **布局耗时占开发 30-50%**：本该专注业务逻辑，却把时间浪费在调整边距和对齐上

### 这个工具能做什么？

**核心价值：用 Figma 替代 WPF 设计器，自动生成布局骨架。**

```
传统开发流程：
设计师交付 Figma 设计稿 
  → 开发者看着设计稿 
  → 在 Visual Studio 设计器中拖拽/手写 XAML 
  → 反复调试对齐 
  → 添加 UI 库组件 
  → 完成 ✅ (耗时 2-3 小时)

使用本工具后：
设计师交付 Figma 设计稿（使用 Auto Layout）
  → 复制 JSON 
  → 工具生成布局骨架 
  → 替换为 UI 库组件 
  → 完成 ✅ (耗时 30 分钟)
...
本工具**不试图**生成完整可用的 UI（那样会失败），而是专注于：

✅ **生成准确的布局骨架**
- 自动识别 Figma Auto Layout → 选择合适的 WPF 容器（Grid/StackPanel/WrapPanel）
- 自动计算 Grid 的行列定义和子元素位置
- 保留设计稿的对齐、间距、嵌套关系

✅ **作为 Figma 和 UI 库的桥梁**
- 生成的是"脚手架"，包含完整的布局结构
- 开发者在此基础上替换为 MaterialDesign、HandyControl 等 UI 库组件
- 保留布局逻辑，自由添加样式和交互

✅ **替代 WPF 设计器**
- Figma 的设计体验远超 Visual Studio 设计器
- 支持 Auto Layout、组件、变体等现代设计工具特性
- 设计师和开发者使用同一个工具，减少沟通成本
...
**推荐的设计方式**：
- ✅ 使用 **Auto Layout**（HORIZONTAL/VERTICAL）而非固定定位
- ✅ 合理设置间距（itemSpacing、padding）
- ✅ 使用对齐属性（主轴对齐、交叉轴对齐）
- ✅ 规范命名图层（会转为注释）

**不推荐的设计方式**：
- ❌ 全部用固定宽高和绝对定位（会转为复杂的 Grid，难以调整）
- ❌ 过度嵌套（虽然能转换，但生成的 XAML 会很复杂）
- ❌ 使用不支持的控件类型（如 VECTOR、COMPONENT）

**建议**：与设计师约定使用 Auto Layout 进行设计，这样既利于响应式布局，也方便转换为 XAML。

### 适用场景

**最适合：**
- 🎯 企业内部工具开发（设计规范统一，布局为主）
- 🎯 快速原型开发（先搭骨架，再完善细节）
- 🎯 使用 UI 库的项目（MaterialDesign、HandyControl、MahApps.Metro）
- 🎯 设计稿频繁变更的项目（重新生成骨架即可）

**不太适合：**
- ❌ 复杂的图形界面（游戏、数据可视化等）
- ❌ 需要像素级精确还原的场景
- ❌ 设计师不会/不愿使用 Auto Layout
